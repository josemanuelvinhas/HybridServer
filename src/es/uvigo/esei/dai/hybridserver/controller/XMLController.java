...
import es.uvigo.esei.dai.hybridserver.model.dao.DBDAO_XML;
...
public class XMLController {
	...
	public XMLController(DB db, HTTPRequest request) {
		...
		this.dao = new DBDAO_XML(db.getUrl(), db.getUser(), db.getPassword());
	}
	...
	private HTTPResponse getResponseDelete(HTTPRequest request) {
		...
		if ((uuid = request.getResourceParameters().get("uuid")) != null) { //Si el recurso es HTML y se la petición contiene el uuid
			try {
	...
	private HTTPResponse getResponsePost(HTTPRequest request) {
		...
		if ((xml = request.getResourceParameters().get("xml")) != null) { //Si la petición tiene el contenido
			try {
				...
				dao.insert(new Document(uuid, xml));
		...
		if (uuid != null) { //Si se pasa el parametro uuid
			...
			try {
				for (Document page : dao.listPages()) {
					content.append("<li><a href=\"/xml?uuid=").append(page.getUUID()).append("\">").append(page.getUUID())
							.append("</a></li>");
				}
